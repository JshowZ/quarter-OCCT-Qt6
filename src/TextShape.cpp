...
#ifdef Q_OS_WIN
    QString str = QString::fromLocal8Bit(src);
...
namespace Base
{
    static std::map<std::string, std::string> g_occt_fonts_maps; 
...
//    }
 ...
 /*   void load_text_shape(const char* text, const char* font, const float text_height, const float thickness, bool is_bold, bool is_italic, TextResult& text_result)
    ...
    std::vector<std::string> TextShape::InitOcctFonts()
    {
        std::vector<std::string> stdFontNames;
    ...
    std::map<std::string, std::string> TextShape::GetOcctFontsMaps()
    {
        return g_occt_fonts_maps;
    }

    bool TextShape::MakeTextShape(const char* text, const char* font, const float textHeight, const float thickness, bool isBold, bool isItalic, TopoDS_Shape& resultShape, double& dTextWidth)
    {
        if (thickness <= 0)
            return false;
